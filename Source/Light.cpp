...
uint32_t Pack(const FVector3& p_toPack)
{
...
Light::Light(FTransform& p_tranform, Type p_type) :
    m_transform(p_tranform), 
	type(static_cast<float>(p_type))
{
}

FMatrix4 Light::GenerateMatrix() const
{
	FMatrix4 result;
...
const FTransform& Light::GetTransform() const
{
	return m_transform;
}
