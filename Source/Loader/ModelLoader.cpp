...
Model* ModelLoader::Create(const std::string& p_filepath)
{
	Model* result = new Model(p_filepath);

	if (__ASSIMP.LoadModel(p_filepath, result->m_meshes, result->m_materialNames))
	{
		return result;
	}
...
void ModelLoader::Reload(Model& p_model, const std::string& p_filePath)
{
