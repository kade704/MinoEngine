...
void LuaGlobalBinder::BindGlobal(sol::state& p_luaState)
{
	...
	p_luaState["Debug"]["Log"] = [](const std::string& p_message) { MINO_LOG(p_message); };
	p_luaState["Debug"]["Warning"] = [](const std::string& p_message) { MINO_WARNING(p_message); };
	p_luaState["Debug"]["Error"] = [](const std::string& p_message) { MINO_ERROR(p_message); };

	p_luaState["Input"]["GetKeyDown"] = [](EKey p_key) { return MINO_SERVICE(InputManager).IsKeyPressed(p_key); };
	p_luaState["Input"]["GetKeyUp"] = [](EKey p_key) { return MINO_SERVICE(InputManager).IsKeyReleased(p_key); };
	p_luaState["Input"]["GetKey"] = [](EKey p_key) { return MINO_SERVICE(InputManager).GetKeyState(p_key) == EKeyState::KEY_DOWN; };

	p_luaState["Input"]["GetMouseButtonDown"] = [](EMouseButton p_button) { return MINO_SERVICE(InputManager).IsMouseButtonPressed(p_button); };
	p_luaState["Input"]["GetMouseButtonUp"] = [](EMouseButton p_button) { return MINO_SERVICE(InputManager).IsMouseButtonReleased(p_button); };
	p_luaState["Input"]["GetMouseButton"] = [](EMouseButton p_button) { return MINO_SERVICE(InputManager).GetMouseButtonState(p_button) == EMouseButtonState::MOUSE_DOWN; };

	p_luaState["Input"]["GetMousePos"] = []()
	{
		auto mousePos = MINO_SERVICE(InputManager).GetMousePosition();
